<template>
  <div class="account-main">
    <div class="account-left">
      <a-list bordered :dataSource="udata" style="height: 632px">
        <a-list-item slot="renderItem" slot-scope="item">
          <a @click="() => changeCurrent(item.id, item.username)">{{
            item.username
          }}</a>
        </a-list-item>
        <div slot="header">沟通列表</div>
      </a-list>
    </div>
    <div class="account-right">
      <div class="account-title">和 {{ currentUsername }} 对话</div>

      <a-spin tip="加载中..." :spinning="spinning">
        <a-card
          style="height: 400px; background-color: #fafafa; overflow: auto"
        >
...
import MsgItem from "@/components/user/MsgItem";
export default {
  components: {
    MsgItem
  },
    ...
    fetchUsers() {
      axios
        ...
        .finally(() => {
          ...
          const id = this.$route.params.idFromTalk;
          const username = this.$route.params.usernameFromTalk;
          if (id && username) {
            this.currentUserId = id;
            this.currentUsername = username;
          }
